...
| `test_executor.py`  | Testa normalização de ações, leitura de arquivo e integração parcial do executor.      |
...
> **Observação:** Os testes de `base_modelos` requerem `sqlalchemy`. Com ele instalado, toda a suíte é executada; do contrário, os testes correspondentes são ignorados automaticamente.
...
3. **Evite efeitos colaterais:** use bancos em memória (`sqlite:///:memory:`) ou arquivos temporários (`tempfile`) para manter o ambiente limpo.
4. **Rode os testes antes de abrir PRs** para garantir regressões zero.
