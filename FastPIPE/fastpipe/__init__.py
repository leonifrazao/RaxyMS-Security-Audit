"""FastPIPE - lightweight service pipes for intra-project function calls."""
...
def _mark_endpoint(func: Callable[..., Any], alias: Optional[str] = None) -> Callable[..., Any]:
    setattr(func, "__fastpipe_endpoint__", alias or func.__name__)
    return func
...
def home(func: Callable[..., Any]) -> Callable[..., Any]:
    """Mark a method as the ``home`` endpoint."""

    return _mark_endpoint(func, "home")


def get(func: Callable[..., Any]) -> Callable[..., Any]:
    """Mark a method as a readable endpoint (name defaults to the method name)."""

    return _mark_endpoint(func, None)


def post(func: Callable[..., Any]) -> Callable[..., Any]:
    """Mark a method as a writable endpoint (name defaults to the method name)."""

    return _mark_endpoint(func, None)


class _ServiceRegistration:
    """Holds metadata about a declared FastPIPE service."""
    ...
    def _register_class(self, cls: type) -> type:
        ...
        for attr_name, member in cls.__dict__.items():
            ...
            if func is None or not hasattr(func, "__fastpipe_endpoint__"):
                continue
            endpoint_name = getattr(func, "__fastpipe_endpoint__", func.__name__)
...
def _annotate_endpoint(func: Callable[..., Any], owner: type, attr_name: str, descriptor: str) -> None:
    setattr(func, "__fastpipe_owner__", owner)
    setattr(func, "__fastpipe_attr__", attr_name)
    setattr(func, "__fastpipe_descriptor__", descriptor)
...
def create(name: str) -> _ServiceRegistration:
    """Create (or retrieve) a FastPIPE service registration for the given name."""
...
def _resolve_registration(name: Optional[str]) -> _ServiceRegistration:
    if name is not None:
        try:
            ...
            raise FastPipeError(f"Service '{name}' was not registered via fastpipe.create().") from exc
    if _LAST_REGISTRATION is None:
        raise FastPipeError("No FastPIPE service has been registered yet.")
...
def run(
    ...
    *,
    ...
    """Launch the given service in daemon mode (defaults to the last declared service)."""
