...
import daemon_service  # noqa: F401 - ensure service starts


def bench_latency(n=200):
    c = fp.connect("demo-service", argumento="ping", timeout=3.0)
    # warmup
    for _ in range(20):
        c.get()
    times = []
...
def bench_throughput(concurrency=16, duration=2.0):
    ...
    def worker(i):
        c = fp.connect("demo-service", argumento="pong", timeout=3.0)
...
if __name__ == "__main__":
    os.environ.setdefault("FASTPIPE_WORKERS", "16")
    lat = bench_latency()
