...
from raxy.core.domain.mailtm import Domain, Account, AuthenticatedSession, Message, MessageAddress
...
class MailTm(BaseAPIClient):
    ...
    def __init__(self, logger: Optional[Any] = None):
        ...
        """
        config = get_config()
        base_url = config.api.mail_tm.url if hasattr(config.api, 'mail_tm') else "https://api.mail.tm"
    ...
    def get_domains(self) -> List[Domain]:
        ...
        def _call():
            return self.executar("GET", "/domains", headers=headers)
        ...
        # Botasaurus pode retornar o JSON já parseado como lista ou dict
        domains_data = []
        if isinstance(response, dict) and "hydra:member" in response:
            domains_data = response['hydra:member']
        elif isinstance(response, list):
            domains_data = response
        ...
        try:
            ...
            self.logger.erro(f"Erro ao parsear domínios: {e}")
            return []
    ...
    def create_account(self, address: str, password: str) -> AuthenticatedSession:
        ...
        def _call():
            return self.executar("POST", "/accounts", json=payload, headers=headers)
    ...
    def get_token(self, address: str, password: str) -> str:
        ...
        def _call():
            return self.executar("POST", "/token", json=payload, headers=headers)
    ...
    def get_me(self, token: str) -> Account:
        ...
        me_data = self.executar("GET", "/me", headers=headers)
    ...
    def delete_account(self, session: AuthenticatedSession) -> None:
        ...
        self.executar("DELETE", f"/accounts/{account_id}", headers=headers)
    ...
    def get_messages(self, token: str, page: int = 1) -> List[Message]:
        ...
        response = self.executar("GET", f"/messages?page={page}", headers=headers)
    ...
    def get_message(self, token: str, message_id: str) -> Message:
        ...
        m = self.executar("GET", f"/messages/{message_id}", headers=headers)
        from_addr = MessageAddress(address=m['from'].get('address', ''), name=m['from'].get('name', ''))
        to_addrs = [MessageAddress(address=t.get('address', ''), name=t.get('name', '')) for t in m.get('to', [])]
    ...
    def mark_message_as_seen(self, token: str, message_id: str, seen: bool = True):
        ...
        response = self.executar("PATCH", f"/messages/{message_id}", json=payload, headers=headers)
        return response
