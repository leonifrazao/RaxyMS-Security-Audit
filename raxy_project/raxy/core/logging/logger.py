...
class RaxyLogger(ILoggingService):
    ...
    def __init__(self, config: Optional[LoggerConfig] = None):
        ...
        self._disabled_handlers: List[LogHandler] = []
    ...
    def mute_console(self) -> None:
        """Desativa saída para console."""
        from .handlers import ConsoleHandler, BufferedHandler
        
        # Identifica handlers de console
        handlers_to_remove = []
        for handler in self.handlers:
            is_console = isinstance(handler, ConsoleHandler)
            # Verifica BufferedHandler também
            if isinstance(handler, BufferedHandler) and isinstance(handler.target, ConsoleHandler):
                is_console = True
                
            if is_console:
                handlers_to_remove.append(handler)
        
        # Move para lista de desabilitados
        for handler in handlers_to_remove:
            self.handlers.remove(handler)
            self._disabled_handlers.append(handler)

    def unmute_console(self) -> None:
        """Reativa saída para console."""
        # Restaura handlers desabilitados
...
class ScopedLogger(ILoggingService):
    ...
    def mute_console(self) -> None:
        """Delegates to parent."""
        self.parent.mute_console()

    def unmute_console(self) -> None:
        """Delegates to parent."""
        self.parent.unmute_console()
